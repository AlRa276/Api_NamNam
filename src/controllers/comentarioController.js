...
const crear =    async (req, res, next) => { try { const c = await comentariosService.crear(Number(req.params.id_receta), req.body); res.status(201).json({ ok: true, id: c.id, mensaje: 'Comentario agregado' }); } catch (e) { next(e); } };
