...
const agregar =  async (req, res, next) => { try { await favoritosService.agregar(Number(req.body.id_usuario), Number(req.params.id_receta)); res.status(201).json({ ok: true, mensaje: 'Agregado a favoritos' }); } catch (e) { next(e); } };
const eliminar = async (req, res, next) => { try { await favoritosService.eliminar(Number(req.body.id_usuario), Number(req.params.id_receta)); res.json({ ok: true, mensaje: 'Eliminado de favoritos' }); } catch (e) { next(e); } };
